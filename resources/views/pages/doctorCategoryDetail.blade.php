...
@endsection
@section('SEO')
<meta name="keywords" content="{{$category->getSeoKeys()}}, Услуги, дармон, darmon service, services, servis, xizmatlar">
<meta name="description" content="{{$category->getSeoDesc()}}">
@endsection
@section('locales')
@php
    $localesAr = [
...
@foreach(config('app.available_locales') as $locale)
    <li><a class="dropdown-item" href="{{ route(\Illuminate\Support\Facades\Route::currentRouteName(), ['locale'=>$locale, 'slug'=>$category->getSlug()]) }}">{{$localesAr[$locale]}}</a></li>
    @endforeach
@endsection
@section('content')
<section class="py-2">
    <div class="container py-5">
        <h2>{{__('Специалисты Darmon Servis')}}: {{$category->getTitle()}}</h2>
        <div class="row">
